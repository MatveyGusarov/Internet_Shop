...
async def end_deliver(message: Message, state: FSMContext):
    ...
    if len(result) == 0:
        ...
        cur.execute(f"UPDATE products SET number_products = {int(local_data[1]) + int(result[0][0])} WHERE id = {int(local_data[0])}")
        conn.commit()
...
async def end(message: Message, state: FSMContext):
    if message.text == 'end':
        ...
        cur.execute(f'''INSERT INTO products VALUES ('{local_data[0]}', '{local_data[1]}', '{local_data[2]}', 
            '{local_data[3]}', '{local_data[4]}', '{local_data[5]}', '{local_data[6]}', '{local_data[7]}')''')
        conn.commit()
        await state.finish()
...
async def end_delete(message: Message, state: FSMContext):
    ...
    if len(result) == 0:
        ...
        await message.answer('One moment!')
        cur.execute(f"DELETE FROM products WHERE id = {int(local_data[0])}")
        conn.commit()
